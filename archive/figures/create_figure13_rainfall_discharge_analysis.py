...
# -*- coding: utf-8 -*-
"""
Figure 13: 降雨強度と放流量の関係分析
降雨強度（60分雨量）を用いた放流量予測のための包括的分析
"""
...
class Figure13RainfallDischargeAnalysis:
    """降雨強度-放流量関係分析クラス"""
    ...
    def load_and_preprocess_data(self):
        ...
        # 必要なカラムの確認
        required_cols = ['時刻', '水位_水位', 'ダム_全放流量', 'ダム_60分雨量']
        ...
        self.df.columns = ['時刻', '水位', '放流量', '降雨強度']
        
        # フィルタリング（水位≥3m、放流量≥150m³/s、降雨強度>0mm/h）
        print("\nフィルタリング前:")
        print(f"  全データ数: {len(self.df)}")
        
        mask = (self.df['水位'] >= 3.0) & (self.df['放流量'] >= 150.0) & (self.df['降雨強度'] > 0.0)
        ...
        # 欠損値の処理
        print("\n欠損値の確認:")
        print(f"  降雨強度: {self.df_filtered['降雨強度'].isna().sum()}件")
        print(f"  放流量: {self.df_filtered['放流量'].isna().sum()}件")
        
        # 欠損値を前方補完
        self.df_filtered['降雨強度'].fillna(method='ffill', inplace=True)
        self.df_filtered['放流量'].fillna(method='ffill', inplace=True)
        
        # 統計情報
        print("\n基本統計:")
        print(self.df_filtered[['水位', '放流量', '降雨強度']].describe())
        
        return self.df_filtered
    
    def analyze_cross_correlation(self):
        """降雨強度と放流量の相互相関分析（0-6時間）"""
        print("\n=== 相互相関分析 ===")
        
        
        # データ準備
        rainfall = self.df_filtered['降雨強度'].values
        ...
        for lag in lags:
            if lag == 0:
                corr = np.corrcoef(rainfall, discharge)[0, 1]
            else:
                # 降雨が先行する場合の相関
                if len(rainfall) > lag:
                    corr = np.corrcoef(rainfall[:-lag], discharge[lag:])[0, 1]
                else:
                    corr = np.nan
        ...
        # 降雨強度レベル別の相関分析
        rainfall_levels = [(0, 10), (10, 20), (20, 30), (30, 50), (50, 100), (100, 200)]
        level_results = []
        
        for min_r, max_r in rainfall_levels:
            mask_level = (rainfall >= min_r) & (rainfall < max_r)
            if mask_level.sum() > 20:  # 最低20データポイント
                rainfall_level = rainfall[mask_level]
                ...
                for lag in range(0, min(31, len(rainfall_level))):  # 最大5時間
                    if lag > 0 and len(rainfall_level) > lag:
                        try:
                            corr = np.corrcoef(rainfall_level[:-lag], discharge_level[lag:])[0, 1]
                ...
                level_results.append({
                    'rainfall_range': f"{min_r}-{max_r}",
                    'optimal_lag_minutes': best_lag * 10,
                    'correlation': best_corr,
                    'data_points': mask_level.sum()
                })
                print(f"  {min_r}-{max_r} mm/h: {mask_level.sum()}件, "
    ...
    def analyze_response_patterns(self):
        ...
        self.TodoWrite_status = 'in_progress'
        print("ToDo更新: 遅延時間別の応答特性分析")
        ...
        for lag_h in lag_hours:
            ...
            if lag_points < len(self.df_filtered) - 1:
                if lag_points == 0:
                    x = self.df_filtered['降雨強度'].values
                    y = self.df_filtered['放流量'].values
                else:
                    x = self.df_filtered['降雨強度'].values[:-lag_points]
    ...
    def build_prediction_models(self):
        ...
        for pred_h in prediction_hours:
            ...
            for i in range(len(self.df_filtered) - pred_points - 6):
                # 入力: 現在〜1時間後の降雨強度（1時間平均）
                rain_current = self.df_filtered['降雨強度'].iloc[i]
                rain_1h_avg = self.df_filtered['降雨強度'].iloc[i:i+6].mean()
                
                # 特徴量: 現在の降雨、1時間平均降雨、現在の放流量
                current_discharge = self.df_filtered['放流量'].iloc[i]
                
                # 出力: pred_h時間後の放流量
                future_discharge = self.df_filtered['放流量'].iloc[i + pred_points]
                
                input_data.append([rain_current, rain_1h_avg, current_discharge])
    ...
    def evaluate_predictions(self):
        ...
        self.TodoWrite_status = 'in_progress'
        print("ToDo更新: 予測精度の評価")
        
        # 降雨強度別の精度評価
        rainfall_bins = [0, 10, 20, 30, 50, 100, 200]
        evaluation_results = {}
        ...
        for pred_h, model_info in self.prediction_models.items():
            ...
            # 降雨強度別評価（1時間平均降雨で分類）
            rain_1h_avg = X_test[:, 1]  # 1時間平均降雨
            
            level_results = []
            for i in range(len(rainfall_bins) - 1):
                mask = (rain_1h_avg >= rainfall_bins[i]) & (rain_1h_avg < rainfall_bins[i+1])
                if mask.sum() > 5:
                    ...
                    level_results.append({
                        'rainfall_range': f"{rainfall_bins[i]}-{rainfall_bins[i+1]}",
                        'n_samples': mask.sum(),
                        'rmse': rmse,
                        'mae': mae,
                        'mape': mape
                    ...
                    print(f"  {rainfall_bins[i]}-{rainfall_bins[i+1]} mm/h: "
    ...
    def analyze_change_rate_correlation(self):
        ...
        for lag in range(0, max_lag + 1):
            ...
            # 変化率計算
            for i in range(interval, len(rainfall) - interval - lag):
                # 降雨強度の変化率
                dR_dt = (rainfall[i + interval] - rainfall[i - interval]) / (2 * interval * 10 / 60)  # mm/h/h
                
                # 放流量の変化率（遅延を考慮）
                if i + lag + interval < len(discharge):
                    dQ_dt = (discharge[i + lag + interval] - discharge[i + lag - interval]) / (2 * interval * 10 / 60)  # m³/s/h
                    
                    correlations_all.append((dR_dt, dQ_dt))
                    
                    # 放流量の変化方向で分類
                    if dQ_dt > 0:
                        correlations_inc.append((dR_dt, dQ_dt))
                    else:
                        correlations_dec.append((dR_dt, dQ_dt))
        ...
        for i in range(interval, len(rainfall) - interval - optimal_lag_idx):
            dR_dt = (rainfall[i + interval] - rainfall[i - interval]) / (2 * interval * 10 / 60)
    ...
    def create_figure13(self):
        """Figure 13: 降雨強度-放流量関係の包括的分析"""
        print("\n=== Figure 13 作成 ===")
        
        self.TodoWrite_status = 'in_progress'
        print("ToDo更新: Figure 13の作成と保存")
        
        # 3行4列のレイアウトに変更（3行目に変化率分析グラフを追加）
        ...
        if hasattr(self, 'correlation_results') and 'by_level' in self.correlation_results:
            ...
            # 降雨強度の中央値を計算
            rainfall_centers = []
            for range_str in level_data['rainfall_range']:
                min_val, max_val = map(float, range_str.split('-'))
                rainfall_centers.append((min_val + max_val) / 2)
            
            ax5.plot(rainfall_centers, level_data['optimal_lag_minutes'], 'bo-', linewidth=2, markersize=8)
            ax5.set_xlabel('降雨強度 (mm/h)')
            ax5.set_ylabel('最適遅延時間 (分)')
            ax5.set_title('降雨強度別の最適遅延時間')
            ax5.grid(True, alpha=0.3)
            ax5.set_xlim(0, max(rainfall_centers) + 10)
            
            # データポイント数を表示
            for i, (x, y, n) in enumerate(zip(rainfall_centers, level_data['optimal_lag_minutes'], level_data['data_points'])):
                ax5.text(x, y + 5, f'n={n}', ha='center', va='bottom', fontsize=8)
        
        # グラフ6: 降雨強度別の予測精度（RMSE）
        ...
        if hasattr(self, 'evaluation_results'):
            ...
            if 1 in self.evaluation_results:
                ...
                # 降雨強度の中央値を計算
                rainfall_centers = []
                for range_str in eval_1h['rainfall_range']:
                    min_val, max_val = map(float, range_str.split('-'))
                    rainfall_centers.append((min_val + max_val) / 2)
                ...
                # RMSE
                p1 = ax6_rmse.plot(rainfall_centers, eval_1h['rmse'], 'ro-', linewidth=2, markersize=8, label='RMSE')
                ax6_rmse.set_xlabel('降雨強度 (mm/h)')
                ax6_rmse.set_ylabel('RMSE (m³/s)', color='r')
                ax6_rmse.tick_params(axis='y', labelcolor='r')
                
                # MAPE
                p2 = ax6_mape.plot(rainfall_centers, eval_1h['mape'], 'bs-', linewidth=2, markersize=8, label='MAPE')
                ax6_mape.set_ylabel('MAPE (%)', color='b')
                ax6_mape.tick_params(axis='y', labelcolor='b')
                
                ax6.set_title('降雨強度別の予測精度（1時間先）')
                ax6.grid(True, alpha=0.3)
                ax6.set_xlim(0, max(rainfall_centers) + 10)
        ...
        # 降雨強度を10mm/hごとに区切ってデータ数をカウント
        rainfall_data = self.df_filtered['降雨強度'].values
        bins = np.arange(0, max(rainfall_data) + 10, 10)
        counts, _ = np.histogram(rainfall_data, bins=bins)
        
        # ビンの中央値を計算
        bin_centers = (bins[:-1] + bins[1:]) / 2
        
        # 棒グラフで表示
        bars = ax7.bar(bin_centers, counts, width=8, alpha=0.7, color='steelblue', edgecolor='black')
        ...
        ax7.set_xlabel('降雨強度 (mm/h)')
        ax7.set_ylabel('データ数')
        ax7.set_title('降雨強度別のデータ分布')
        ax7.grid(True, alpha=0.3)
        ax7.set_xlim(-5, max(bin_centers) + 10)
        
        # 統計情報を追加
        total_data = len(rainfall_data)
        mean_rainfall = np.mean(rainfall_data)
        median_rainfall = np.median(rainfall_data)
        
        stats_text = f'総データ数: {total_data:,}\n平均: {mean_rainfall:.1f} mm/h\n中央値: {median_rainfall:.1f} mm/h'
        props = dict(boxstyle='round', facecolor='white', alpha=0.8)
        ...
        if hasattr(self, 'change_rate_results'):
            ...
            ax10.set_xlabel('降雨強度変化率 (mm/h/h)')
        ...
        if hasattr(self, 'change_rate_results') and hasattr(self, 'lag_correlation_results'):
            ...
            # 降雨強度ビンごとの相関分析
            rainfall_bins = [0, 5, 10, 15, 20, 30, 50]
            bin_correlations = []
            
            for i in range(len(rainfall_bins) - 1):
                min_r, max_r = rainfall_bins[i], rainfall_bins[i+1]
                
                # 該当する降雨強度のデータを抽出
                bin_mask = (self.df_filtered['降雨強度'].iloc[6:-6-int(optimal_lag_all/10)] >= min_r) & \
                          (self.df_filtered['降雨強度'].iloc[6:-6-int(optimal_lag_all/10)] < max_r)
                
                if bin_mask.sum() > 20:
                    bin_data = self.change_rate_results[bin_mask.values]
                    if len(bin_data) > 10:
                        corr = np.corrcoef(bin_data['dR_dt'], bin_data['dQ_dt'])[0, 1]
                        bin_correlations.append({
                            'rainfall_center': (min_r + max_r) / 2,
                            'correlation': corr,
            ...
            if bin_correlations:
                ...
                ax11.bar(bin_df['rainfall_center'], bin_df['correlation'], 
                        width=4, alpha=0.7, color='green', edgecolor='black')
                
                # データ数を表示
                for i, row in bin_df.iterrows():
                    ax11.text(row['rainfall_center'], row['correlation'] + 0.02,
                            f"n={row['n_points']}", ha='center', va='bottom', fontsize=8)
                
                ax11.set_xlabel('降雨強度 (mm/h)')
                ax11.set_ylabel('変化率の相関係数')
                ax11.set_title('降雨強度別の変化率相関')
        ...
        if hasattr(self, 'change_rate_results'):
            ...
            # 線形回帰
            from sklearn.linear_model import LinearRegression
            X = data['dR_dt'].values.reshape(-1, 1)
            y = data['dQ_dt'].values
        ...
        # 全体タイトル
        fig.suptitle('Figure 13: 降雨彷度を用いた放流量予測の包括的分析', fontsize=16, y=0.995)
        ...
        output_path = f"figures/figure13_rainfall_discharge_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        plt.savefig(output_path, dpi=150, bbox_inches='tight')
        print(f"\nFigure 13 を保存しました: {output_path}")
    ...
    def save_analysis_results(self):
        ...
        # 1. 相関分析結果
        corr_file = f"rainfall_discharge_correlation_{timestamp}.csv"
        ...
        model_file = f"rainfall_discharge_models_{timestamp}.csv"
        ...
        for h, eval_df in self.evaluation_results.items():
            eval_file = f"rainfall_discharge_evaluation_{h}h_{timestamp}.csv"
            eval_df.to_csv(eval_file, index=False, encoding='utf-8-sig')
            print(f"{h}時間先予測の評価結果を保存: {eval_file}")

def main():
    """メイン処理"""
    print("降雨強度-放流量関係分析プログラム")
    print("=" * 60)
    
    analyzer = Figure13RainfallDischargeAnalysis()
