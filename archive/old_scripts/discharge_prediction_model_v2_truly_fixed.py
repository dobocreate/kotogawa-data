...
class DischargePredictionModelV2:
    ...
    def get_dynamic_delay(self, state, rainfall_current, rainfall_history):
        ...
        if rain_increasing and (state <= 0):
            # 降雨が急増した場合は即座に反応（状態に関わらず）
            return self.params['delay_onset']
        elif state == 1:
            # 増加継続中 → 通常の遅延
            return self.params['delay_increase']
        elif rainfall_current < self.params['rain_threshold_low'] and state <= 0:
            # 低降雨かつ非増加状態 → 減少開始の遅延
    ...
    def predict_discharge_change(self, state, rainfall_intensity, current_discharge):
        ...
        if rain_category == 'low':
            if state <= 0:  # 減少中または定常
                ...
                # 増加中でも低降雨なら減速
                base_rate = self.params['rate_increase_low'] * 0.3
    ...
    def predict(self, current_time, historical_data, prediction_hours=3, 
        ...
        # 過去の降雨履歴（現在時刻まで）
        past_rainfall_history = rainfall_history.copy()
        ...
        for step in range(1, int(prediction_hours * 6) + 1):
            ...
            # 動的遅延時間の取得（過去の降雨データのみ使用）
            delay = self.get_dynamic_delay(predicted_state, current_rainfall, past_rainfall_history)
            
            # 遅延を考慮した降雨強度の計算
            delay_steps = delay // 10
            
            # 重要な修正：遅延を考慮した時刻を計算
            effective_time_offset = step - delay_steps - 1
            
            if effective_time_offset >= 0:
                # 予測期間内のデータを参照している場合
                if effective_time_offset < step and step <= 6:
                    # 1時間以内の予測データを使用
                    effective_rainfall = future_rainfall if effective_time_offset == step - 1 else 0
                else:
                    # 予測期間外または1時間超
                    effective_rainfall = 0
            else:
                # 過去のデータを参照している場合
                hist_idx = -effective_time_offset - 1
                if hist_idx < len(past_rainfall_history):
                    effective_rainfall = past_rainfall_history.iloc[-(hist_idx + 1)]
                else:
                    effective_rainfall = past_rainfall_history.iloc[0]
